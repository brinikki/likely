export default function StandardCard({ card, title, text, total, index  }) {
    return (
        <div className="flex flex-col items-center text-center max-w-x1 w-full gap-3">
            <p className="text-xs tracking-widest uppercase text-stone-600 mb-6">{String(index + 1).padStart(2, '0')} /{total}</p>
            <h2 className="font-light text-4xl md:text-5xl leading-tight text-stone-100 mb-5 max-w-2xl">{card.title}</h2>
